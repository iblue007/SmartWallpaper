...
/**
 * @Class: ImageLoaderPhonePicker
 * @Description:
 * @author: lling(www.liuling123.com)
 * @Date: 2015/11/5
 */
public class ImageLoaderPhonePicker {
    ...
    private static ImageLoaderPhonePicker mInstance;
    private int mWidth;

    private ImageLoaderPhonePicker() {
        init();
    }
    ...
    public static synchronized ImageLoaderPhonePicker getInstance() {
        if(mInstance == null) {
            mInstance = new ImageLoaderPhonePicker();
        }
        return mInstance;
    }
    ...
    private void initDiskCache() {
        try {
            File cacheDir = OtherUtils.getDiskCacheDir(Global.getContext(), "images");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            mDiskLruCache = DiskLruCache.open(cacheDir, OtherUtils.getAppVersion(Global.getContext()),
                    1, 15 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
